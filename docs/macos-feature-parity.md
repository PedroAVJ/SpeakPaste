...
| Concurrent work | Let another dictation start while an earlier one transcribes | The microphone is released at every pause, Scribe requests are admission-limited, and completed live dictations are delivered in spoken order. Imports do not block that queue. Delivery is no longer a preference: while a dictation is open — connecting, recording, or resting — nothing reaches the cursor, and every banked segment is delivered in spoken order the moment fn closes it. Pause transcribes eagerly, so the End after a pause is usually instant. |
