...
| Other audio | Make speaking easy to hear without controlling media apps | While capture is live, both Mac and Continuity sources use Apple's speech-aware advanced ducking at the mid level. AVCapture-backed recording uses a short-lived VPIO companion; Continuity normally uses its recording VPIO. Failure is surfaced as a nonfatal warning, and previous configuration is restored before recorder stop or transcription. SpeakPaste never sends player commands or writes system volume. |
