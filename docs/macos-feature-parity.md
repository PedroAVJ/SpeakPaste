...
| Capture status | A small recording surface that reports, rather than commands | One wordless, click-through Liquid Glass capsule spring-morphs through connecting, listening, releasing, and transcribing. Listening shows a real voice waveform; transcription shows directional heuristic progress capped below completion rather than a spinner or false percentage. Capture states take priority over concurrent transcription. The capsule has no controls, is never visible at idle, auto-dismisses after work finishes, and never carries success, error, held-output, setup, model, or network notifications. A stuck connecting state is hidden after 20 seconds, releasing after 15, and continuously visible transcription after 90; the dashboard and menu bar own long-lived state. High-priority VoiceOver announcements describe consequential phase changes. |
| Interaction sounds | Confirm state without demanding a glance | Preloaded, quiet earcons share one timbre family: a rising capture-live cue and falling release mirror, plus related tones reserved for verified delivery and gentle errors. The capture cues follow the recorder's proven state, and the whole family obeys the Sounds setting. |
