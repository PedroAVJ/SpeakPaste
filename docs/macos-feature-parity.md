...
- The Continuity microphone must be completely released before any network work
  begins.
- A transcript may enter only the exact text field where recording began. If
  that cannot be proved, the text waits or remains on the clipboard.
...
| --- | --- | --- |
| Global capture | Start and stop without bringing the app forward | Bare right Command toggles capture globally; ordinary Command shortcuts pass through. Escape and visible Cancel controls use the same guard: connecting and recordings under 30 seconds cancel immediately; at 30 seconds or longer the first request arms a three-second confirmation and the second discards. Companion chords fire once per physical keypress, never on OS key-repeat. |
| Capture status | A small recording surface with useful controls | Nonactivating HUD with WAIT, SPEAK NOW, releasing, transcribing, held, done, and error states; Start while optionally kept visible at idle only when capture prerequisites are ready, otherwise SETUP NEEDED; Stop and Cancel during capture; live level/timer; drag-to-dock at all four edges; vertical reflow at the left and right; matching menu-bar state; high-priority VoiceOver announcements for consequential phase changes. |
...
| Microphones | External-device choice and troubleshooting | Exact Core Audio transport classification, ranked user-approved fallback list, gain display, connect latency, live level, and a three-second record/playback test. No guessed Mac-mic fallback. |
...
| Long dictation | Longer sessions with visible limits | Twenty-minute hard limit, one-minute warning, no-audio warning, automatic safe finalization, and guarded cancellation after 30 seconds. Short finalized recordings are still sent to Scribe; there is no arbitrary minimum-duration discard. |
...
| Concurrent work | Let another dictation start while an earlier one transcribes | The microphone is released immediately, Scribe requests are admission-limited, and completed live dictations are delivered in spoken order. Imports do not block that queue. |
| Destination safety | Avoid pasting into the wrong place | SpeakPaste captures the exact focused AX element at start, rechecks that same object inside a serialized delivery gate, refuses secure fields, and holds text when focus changed. App identity, labels, and geometry are never treated as proof of the destination. |
| Delivery compatibility | Work across native, web, and terminal-style fields | Target-app Paste menu first, layout-aware Command-V fallback, optional chunked Unicode type-out, clipboard-only mode, and clipboard restoration after an exact verified insertion. Every pasteboard writer participates in the same serialized transaction, including fallback and explicit History/held copies. Target identity is rechecked at every slow/queued side-effect boundary; partial type-out is never automatically replayed. Unconfirmed delivery remains visibly unconfirmed and on the clipboard. |
| Return to destination | Recover when the user switched away | Same-session held text auto-delivers only when the exact field returns. An opaque field gets at most one automatic insertion attempt: an unconfirmed write is suspended to prevent duplicate text. Recovered text from a prior launch requires the user to focus a destination and press the explicit release shortcut; possibly-delivered text additionally requires a one-shot dashboard authorization for exactly the reviewed queue. |
...
| Per-app behavior | Different apps need different insertion rules | Durable rules keyed by bundle identifier select automatic, Paste-menu, type-out, or clipboard-only delivery; they can opt into context and explicitly confirmed auto-send. Auto-send is never used for delayed held text. |
...
| Local cleanup | Predictable formatting without another model | Scribe's clean-speech option plus local spoken punctuation/new-line commands, deterministic replacements, and cursor-aware spacing/capitalization. |
...
- A restored transcript cannot auto-deliver after relaunch because an archived
  accessibility object cannot prove field identity.
