...
| Other audio | Make speaking easy to hear without stopping media | Once capture is live, both sources use the same independent output fade: about 16 dB down over 400 ms, then back over 900 ms at every recording exit. Playback keeps running and no extra VPIO is created. A crash-recovery lease restores only a volume SpeakPaste still owns; a user volume change or output-route change wins immediately, and unsupported outputs fail open unchanged. |
...
- Controlling unrelated media playback. SpeakPaste never sends play/pause or
  ...
  crash-recoverable, and immediately yields to a user volume or route change.
