...
| Capture status | A small recording surface that reports, rather than commands | Compact click-through Liquid Glass indicator for Connecting, Listening, Releasing, and Transcribing, with source, live level, and recording timer limited to live capture plus restrained indeterminate activity while text is pending. Capture states take priority over concurrent transcription. It has no controls, is never visible at idle, and never carries success, error, held-output, setup, model, or network notifications. A stuck Connecting state is hidden after 20 seconds and Releasing after 15; the dashboard and menu bar own long-lived state. High-priority VoiceOver announcements describe consequential phase changes. |
