...
| Microphones | External-device choice and troubleshooting | Exact Core Audio transport classification, per-dictation semantic Mac/iPhone source chosen by the key pressed, ranked user-approved fallback list within that source, gain display, connect latency, live level, and a three-second record/playback test. The user owns macOS Mic Mode selection; SpeakPaste opens the system chooser, observes preferred versus active mode truthfully, and never writes the global preference. No cross-mode fallback, and no source is remembered between dictations or across launches. There is no mid-recording handover: switching source is pause-then-resume, and the other source key only nudges while a microphone is hot. |
| Continuity reliability | Do not record through the iPhone wake-up gap | Continuity records through Mic-Mode-compatible Voice Processing I/O. A different output is joined with the chosen iPhone input in one nonpersistent process-private aggregate whose topology and selected-input channel map are verified before capture; global default devices are never changed. Readiness is bounded and requires a steady, audible sample stream, plus Voice Isolation actually active when that is the user's preference. Pause, End, Escape, errors, sleep, disconnect, and Quit synchronously remove the tap, stop VPIO, and destroy the private route before batch transcription. |
| Other audio | Make speaking easy to hear without controlling media apps | While capture is live, both Mac and Continuity sources use Apple's speech-aware advanced ducking at the mid level. The Mac recorder keeps AVCapture and adds a short-lived VPIO companion; Continuity uses its recording VPIO. Failure is surfaced as a nonfatal warning, and previous configuration is restored before recorder stop or transcription. SpeakPaste never sends player commands or writes system volume. |
...
| Concurrent work | Let paused segments transcribe without splitting the dictation | The microphone is released at every pause and Scribe requests are admission-limited. Imports do not block live work. Delivery is no longer a preference: while a dictation is open — connecting, recording, or resting — nothing reaches the cursor. Fn closes one atomic delivery batch; it waits for every banked segment, seam-fits them in spoken order, and crosses the output boundary once, so a completed prefix can never land ahead of a slower suffix. Pause transcribes eagerly, so End after a pause is usually instant. |
| Destination behavior | Follow the user's current focus | Inside the serialized delivery gate, SpeakPaste resolves the currently focused application and uses it even when Accessibility exposes no writable node or a replacement Electron/Chromium proxy. Record-start destination identity, secure-field classification, AX object equality, and click/keystroke generations do not veto delivery. Only the absence of an external focused application selects clipboard fallback. |
...
- Controlling unrelated media playback or system volume. SpeakPaste uses only
  Apple's process-scoped speech-aware ducking while its microphone is live; it
  does not send play/pause commands or write a user's volume setting.

The four fixed dictation keys are retained as SpeakPaste's interaction model
