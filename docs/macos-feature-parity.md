...
| Capture status | A small recording surface that reports, rather than commands | Compact click-through indicator for Connecting, Listening, and Releasing only, with source, live level, and recording timer. It has no controls, is never visible at idle, and never carries transcription, success, error, held-output, setup, model, or network notifications. A stuck Connecting state is hidden after 20 seconds and Releasing after 15; the dashboard and menu bar own long-lived state. High-priority VoiceOver announcements describe consequential phase changes. |
