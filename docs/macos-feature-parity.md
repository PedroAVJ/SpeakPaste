...
| Destination safety | Avoid pasting into the wrong place | SpeakPaste captures the exact focused AX element at start, rechecks it inside a serialized delivery gate, refuses secure fields, and holds text when focus changed. If Chromium replaces only the Accessibility proxy, continuity requires the same window, editable role, strong identifier or substantially overlapping frame, and no intervening mouse, key, or scroll input. App identity alone is never proof of the destination. |
...
| Return to destination | Recover when the user switched away | Same-session held text auto-delivers only when the exact field returns. Once one chunk is held, later spoken-order chunks pinned to that exact AX element chain-hold explicitly and the whole run returns as one ordered insertion; an earlier uncertain member blocks the entire exact-field run from skipping ahead. The first held chunk may back the clipboard through a private claim; later chunks never overwrite it repeatedly, and ownership transfers to a sole remainder only while that exact claim is still live. Relaunch and user clipboard changes are reconciled from the claim rather than inferred from queue size. An opaque field gets at most one automatic insertion attempt: an unconfirmed write is suspended to prevent duplicate text. Recovered text from a prior launch requires the user to focus a destination and press the explicit release shortcut; possibly-delivered text additionally requires a one-shot dashboard authorization for exactly the reviewed queue. |
...
| Local cleanup | Predictable formatting without another model | Scribe's clean-speech option plus local spoken punctuation/new-line commands, deterministic replacements, and cursor-aware spacing/capitalization. Intrinsic cleanup is prepared when Scribe returns, but each chunk's seam is fitted only inside the serialized paste transaction against the exact field's live pre-caret text. Ordered held runs fold one chunk at a time, preventing stale record-start snapshots, `word.Next` collisions, duplicate spaces, and broken sentence casing. |
