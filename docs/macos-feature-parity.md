...
| Continuity reliability | Do not record through the iPhone wake-up gap | Continuity first uses Mic-Mode-compatible Voice Processing I/O. A different output is joined with the chosen iPhone input in one nonpersistent process-private aggregate whose directions, topology, and selected-input channel map are verified before capture; global default devices are never changed. If macOS rejects that private VPIO route, the exact chosen iPhone is captured through AVCapture instead. Both routes use bounded readiness with a steady, audible sample stream and require Voice Isolation actually active when that is the user's preference. Pause, End, Escape, errors, sleep, disconnect, and Quit synchronously release the active route before batch transcription. |
| Other audio | Make speaking easy to hear without stopping media | Once capture is live, both sources use the same independent output fade: 16 dB down over 400 ms, then back over 900 ms at every recording exit. Playback keeps running and no extra VPIO is created. A private fsynced write-ahead receipt has no expiry, follows the old output by device identity through route changes or disconnects, and is removed only after complete restoration is read back. Fine user volume changes win through Core Audio listeners; outputs without writable scalar controls and exact scalar/dB translation fail open unchanged. |
...
- Controlling unrelated media playback. SpeakPaste never sends play/pause or
  per-app volume commands. Its temporary output fade is bounded to live capture,
  durably crash-recoverable, and immediately yields to a user volume change while
  restoring the old owned output independently after a route change.
